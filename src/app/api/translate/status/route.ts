...
import { translateStatusQuerySchema, zodErrorDetails } from "@/lib/api-schemas";
import TranslationSessionManager from "@/lib/translation-session-manager";

// GET /api/translate/status — List active translations for a session
export async function GET(req: NextRequest) {
    const parsed = parseSearchParams(req.nextUrl.searchParams, translateStatusQuerySchema);
    if (!parsed.success) {
